...
public class AuthorInfoCacheManager {
    ...
    @Cacheable(cacheManager = CacheConst.REDIS_CACHE_MANAGER,
            value = CacheConst.AUTHOR_INFO_CACHE_NAME, unless = "#result == null")
    public AuthorInfoDto getAuthor(Long userId) {
        QueryWrapper<AuthorInfo> queryWrapper = new QueryWrapper<>();
        queryWrapper
                .eq(DatabaseConst.AuthorInfoTable.COLUMN_USER_ID, userId)
    ...
    @CacheEvict(cacheManager = CacheConst.REDIS_CACHE_MANAGER,
            value = CacheConst.AUTHOR_INFO_CACHE_NAME)
